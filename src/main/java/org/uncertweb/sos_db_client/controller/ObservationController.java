...
import org.uncertweb.sos_db_client.view.ObservationView;
 ...
 */
public class ObservationController implements IController {

	private static final Logger LOG = LogManager.getLogger(ObservationController.class);
	 ...
	 */
	public ObservationController(ObservationView view, MainModel model) {
		...
		public void actionPerformed(ActionEvent event) {
			...
			SwingWorker<Boolean, Void> swingWorker = new SwingWorker<Boolean, Void>() {
				...
				protected Boolean doInBackground() throws Exception {
					...
					List<String> params = view.getObservationParams();

					model.importIntoDatabase("observation", fileUris, params, true);
				...
				protected void done() {
					...
					if(isDone) {
						EventBus.publish(new InformationMessageEvent("Observation(s) imported."));
					}
